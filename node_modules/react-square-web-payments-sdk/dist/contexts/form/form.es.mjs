...
function T({ applicationId: e, locationId: o, children: u, overrides: l, ...r }) {
  const [a, m] = n.useState(), [f] = n.useState(
  ...
  const R = b(async (t) => {
    if (t.status !== "OK" || !r.createVerificationDetails) {
      await r.cardTokenizeResponseReceived(t);
      return;
    }
    const s = await a?.verifyBuyer(String(t.token), r.createVerificationDetails());
    await r.cardTokenizeResponseReceived(t, s);
  });
  if (!e || !o)
    return /* @__PURE__ */ n.createElement(w, null);
  if (!a)
    return null;
  const d = {
  ...
  return /* @__PURE__ */ n.createElement(i.Provider, { value: d }, u);
}
const x = () => {
