export default async function handler(req, res) {
  ...
  try {
    ...
    // Save to Airtable
    const tableName = 'Part-Time Academy Applications';
    ...
    if (!airtableResponse.ok) {
      ...
      console.error('Airtable error (submit-parttime-academy):', airtableResponse.status, errorData);
    ...
    try {
      if (!process.env.RESEND_API_KEY) {
        console.warn('RESEND_API_KEY missing - skipping email notification (submit-parttime-academy)');
      ...
      const emailResponse = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          subject: '⚡ New Part-Time Academy Application',
          html: `
<!DOCTYPE html>
<html>
<head>
